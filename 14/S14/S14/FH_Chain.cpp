...
FH_Debug_Log my_Debug("FH_Data.txt");
...
FH_Chain::~FH_Chain()
{
	...
	if (m_next != NULL)
	{
		my_Debug << (int)m_next << " have been deleted!\n";
		if (my_Debug.classStates[(int)m_next] == HaveNewed)
			my_Debug.classStates[(int)m_next] = HaveDeleted;
		else
			my_Debug.classStates[(int)m_next] = DeleteWithoutNew;
	}
	else
	{
		my_Debug.clear();
...
void FH_Chain::CreateNode()
{
	...
	if (my_Debug.classStates[(int)m_previous] == NotNewed)
		my_Debug.classStates[(int)m_previous] = HaveNewed;
	else
		my_Debug.classStates[(int)m_previous] = DoubleNewed;
	my_Debug << (int)m_previous << " have been newed!\n";
